...
data_fmt <- data %>%
  ...
  dplyr::select(congestive_heart_failure, cardiac_arrhythmia, valvular_disease,
...
model = Mclust(data_fmt, G = 2:15, modelNames = c("EII"),) # Default is G = 1:9, need to set this manually, can also specify certain type of model (E.g., EII, VII)
...
model1 = Mclust(data_fmt, G = 9, modelNames = c("EII"))
...
#%%
#%% USING LPA FOR LCA RESULTS
DATA <- df_means
data_fmt <- DATA*100

model2 = Mclust(data_fmt, G=5:15) # Exploration
plot(model2$BIC)


model1 = Mclust(data_fmt, G = 7, modelNames = c("VEV"))
testa <- c(model1$classification)
model_means <- model1$parameters$mean
# model_means <- model_means * 100 # Scale everything by 100
# model_means <- model_means + 50 # Add 50 to all values as base
colnames(model_means) <- paste("Endotype_", seq(1, ncol(model_means)), sep="")
